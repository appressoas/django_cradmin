...
class TestTarget(test.TestCase):
    def test_dom_id_default(self):
        selector = htmls.S(target_renderer.Target().render(request=mock.MagicMock()))
        self.assertEqual(
            'django_cradmin_multiselect2_select_target',
            selector.one('form')['id'])

    def test_dom_id_custom(self):
        selector = htmls.S(target_renderer.Target(dom_id='customid').render(request=mock.MagicMock()))
        self.assertEqual(
            'customid',
            selector.one('form')['id'])
    ...
    def test_form_action_default(self):
        ...
        selector = htmls.S(target_renderer.Target().render(request=mockrequest))
        self.assertEqual(
            '/form/action',
            selector.one('form')['action'])

    def test_form_action_custom_parameter(self):
        selector = htmls.S(target_renderer.Target(form_action='/some/view').render(request=mock.MagicMock()))
        self.assertEqual(
            '/some/view',
            selector.one('form')['action'])
    ...
    def test_form_action_override(self):
        ...
        selector = htmls.S(MyTarget().render(request=mock.MagicMock()))
        self.assertEqual(
            '/custom',
            selector.one('form')['action'])
    ...
    def test_without_items_text_custom_parameter(self):
        selector = htmls.S(target_renderer.Target(
            without_items_text='Please make a selection').render(request=mock.MagicMock()))
        self.assertEqual(
    ...
    def test_with_items_title_custom_parameter(self):
        selector = htmls.S(target_renderer.Target(
            with_items_title='Test title').render(request=mock.MagicMock()))
        self.assertEqual(
            'Test title',
            selector.one('.django-cradmin-multiselect2-target-title').alltext_normalized)
    ...
    def test_submit_button_text_custom_parameter(self):
        selector = htmls.S(target_renderer.Target(submit_button_text='Perform').render(request=mock.MagicMock()))
        self.assertEqual(
            'Perform',
            selector.one('button[type="submit"]').alltext_normalized)
...
class TestManyToManySelectTarget(test.TestCase):
    def test_button_angularjs_directive(self):
        selector = htmls.S(target_renderer.ManyToManySelectTarget(
            target_formfield_id='testid').render(request=mock.MagicMock()))
        self.assertEqual(
            '{"fieldid": "testid"}',
