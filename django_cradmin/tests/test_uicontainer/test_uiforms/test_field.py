...
class HelperMixin(object):
    ...
    def single_field_formrenderable_factory(self, field, formvalue=None):
        ...
        formrenderable = uicontainer.uiforms.form.FormRenderable(
            form=form,
            children=[uicontainer.uiforms.field.FieldWrapperRenderable(fieldname='testfield')]
        ).bootstrap()
...
class TestAutomaticDjangoFieldRenderable(test.TestCase, HelperMixin):
...
class TestFieldWrapperRenderable(test.TestCase, HelperMixin):
    def test_stuff(self):
        selector = self.single_field_formrenderable_htmls(field=forms.CharField())
