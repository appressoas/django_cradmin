...
the page with the list, or lists, is rendered CRadmin uses the plural verbose name as the page title. For this guide we
have very easy verbose names, so we can use what is automaticly generated by Django, which is the field name with a
